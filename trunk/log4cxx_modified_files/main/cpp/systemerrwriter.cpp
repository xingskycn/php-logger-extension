...
char *SystemErrWriter::lastMessageAscii = NULL;
...
bool SystemErrWriter::hasMessage() {
	return lastMessage.length() > 0;
}

char* SystemErrWriter::getMessage() {
	if (lastMessageAscii != NULL) {
		delete[] lastMessageAscii;
	}
	std::string localErrorMsg;
	Transcoder::encodeUTF8(lastMessage, localErrorMsg);
	
	lastMessageAscii = new char[localErrorMsg.length() + 1];
	std::strcpy(lastMessageAscii, (char *)localErrorMsg.c_str());

	lastMessage.clear();

	return lastMessageAscii;
}

void SystemErrWriter::storeLastMessage(const LogString& str) {
